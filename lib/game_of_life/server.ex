defmodule GOL.Server do
  ...
  def init(_opts) do
    {:ok,
     %{
       # TODO: move this somewhere else, where user can specify the initial config
       cell_to_pid: %{
  ...
  def handle_call({:get_live_coordinates}, _from, %{cell_to_pid: cell_to_pid} = state) do
    live_coordinates =
      Enum.filter(cell_to_pid, fn {_coordinate, pid} ->
      ...
      case Map.fetch(cell_to_pid, coordinates) do
        :error ->
          if has_at_least_on_live_neighbour(coordinates, cell_to_pid) do
            pid =
              GenServer.start(Cell, [elem(coordinates, 0), elem(coordinates, 1), :dead])
              |> elem(1)
